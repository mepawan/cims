												...
												if($conversation){
													...
													array_walk($conversation, function($conv) use(&$active_convs,&$pending_convs){
														...
														if($conv['status'] == 'pending'){
															...
															$pending_convs .= '<a class="btn btn-success accept-conv" href="'.ci_base_url().'provider/messages/accept/'.$conv['id'].'">Accept</a>';
															$pending_convs .= '<a class="btn btn-danger deny-conv" href="'.ci_base_url().'provider/messages/deny/'.$conv['id'].'">Deny</a></li>';
														} else if($conv['status'] == 'active'){
															$active_convs .= '<li><a class="active-conv" href="'.ci_base_url().'provider/conversation/'.$conv['uid1'].'">'.$conv['first_name'] . ' ' . $conv['last_name'].'</a></li>';
														} 
													});
											?>
